...
import * as ack from '../ack-node'
import { routesToArray } from './routesToArray.function'
...
export const main = function(site, options){
  const routes = routesToArray(site.routeLog)
  return testRoutes(routes, options)
}

function testRoutes(routes:any[], options){
  const passing = []
  const failing = []
  const errs = []
  let successCount = 0
...
function getRoutePath(route,sample:any={}){
...
function callbackTimeout(promise,timeout){
  return function(callback){
    ...
    setTimeout(function(){
      const err:any = new Error('Error: Timeout of '+timeout+'ms exceeded. For async tests and hooks, ensure "done()" is called; if returning a Promise, ensure it resolves.')
